...
- [ ] Créer le repo GitHub `atconseil/testvault` (privé en phase 0, public à la GA)
- [ ] Initialiser pnpm workspaces + Turborepo + TypeScript strict (`tsconfig.base.json`)
- [ ] Créer la structure de dossiers `apps/`, `packages/`, `tools/`, `docs/` conforme au plan
- [ ] Configurer **Biome** (lint + format unifiés) à la racine via `biome.json`, branché en CI bloquante
- [ ] Ajouter `gitleaks` en pre-commit hook
- [ ] Créer `.gitignore`, `LICENSE` (Apache 2.0 pour packages publics, `LICENSE-PROPRIETARY` pour extension), `CODE_OF_CONDUCT.md`

**Done quand :**
- [ ] `pnpm install` à la racine fonctionne sans warning
- [ ] `pnpm turbo build` lance un build vide mais réussit
- [ ] Le repo est cloneable et navigable

### T-0.2 — Mettre en place la CI GitHub Actions de base 🔴

📚 `plan.md` §10.1, §10.2

- [ ] Workflow `ci-pr.yml` : checkout → setup Node 22 → pnpm install (frozen) → lint → typecheck → test → build
- [ ] Workflow `ci-main.yml` : tout le ci-pr + build VSIX dry-run
- [ ] Configurer Dependabot (security + patch updates auto-merge ; minor en PR review)
- [ ] Configurer le badge CI dans le `README.md`

**Done quand :**
- [ ] Une PR triviale (typo dans README) déclenche la CI verte
- [ ] La CI bloque si lint ou typecheck échoue (vérifier avec un fail volontaire qu'on revert ensuite)

### T-0.3 — Créer le package `testvault-types` 🔴

📚 `plan.md` §1.1, `spec.md` §6

- [ ] Créer `packages/testvault-types/` avec ses interfaces TypeScript : `TestVaultTestCase`, `TestVaultTestPlan`, `TestVaultTestSet`, `TestVaultPrecondition`, `TestVaultTestExecution`, `TestVaultTestCaseVersion`, `TestVaultAuditLog`
- [ ] Définir les types JSON sérialisés : `TestStep`, `TestStepResult`, `EvidenceRef`
- [ ] Définir les types de configuration : `OrgConfig`, `LLMProviderConfig`, `ProjectConfig`, `UserPreferences`, `EncryptedApiKey`
- [ ] Définir les enums et union types : `TestStatus`, `LLMProviderType`, `AuditOperation`
- [ ] Tests : valider via Zod chaque interface a un schema runtime correspondant

**Done quand :**
- [ ] `pnpm --filter @atconseil/testvault-types build` produit `dist/`
- [ ] Tous les types sont exportés depuis `index.ts`
- [ ] Couverture des schémas Zod ≥ 95%

### T-0.4 — Créer l'extension Argos minimale (Hub vide)

📚 `plan.md` §2.1

- [ ] Créer `apps/argos-extension/` avec `vss-extension.json` configuré : publisher=`ATConseil`, name=`Argos`, targets Cloud + Server `[18.0,)`
- [ ] Implémenter un Hub minimal qui dit juste "Argos — Coming soon" avec layout Fluent UI 2
- [ ] Configurer webpack/vite pour le bundle
- [ ] Tester l'init du SDK : `SDK.init()` puis `SDK.notifyLoadSucceeded()`
- [ ] Builder un VSIX via `tfx-cli`

**Done quand :**
- [ ] Le VSIX est généré sans erreur
- [ ] Le Hub s'affiche correctement dans une org de test ADO Cloud (installation manuelle preview privée)
- [ ] Le Hub s'affiche aussi dans une instance ADO Server 2022 self-hosted (validation manuelle)

### T-0.5 — Détection runtime Cloud vs Server 🟡

📚 `plan.md` §2.2 — bloque T-3.x (features dépendantes)

- [ ] Créer `packages/testvault-sdk/src/runtime-detection.ts` avec la fonction `detectEnvironment()`
- [ ] Tests unitaires couvrant : URL `*.dev.azure.com`, `*.visualstudio.com`, URL custom Server, edge cases (URL malformée, network down)
- [ ] Mock du SDK ADO via msw

**Done quand :**
- [ ] `detectEnvironment()` retourne le bon type sur les 3 cas (cloud, server, error)
- [ ] Couverture ≥ 95%
...
- [ ] Workflow `release-publish.yml` configuré (steps : build → SBOM → package VSIX → publish)
- [ ] PAT Marketplace stocké en GitHub Secret
- [ ] Publication réussie d'une v0.0.1-preview en mode privé (visibilité limitée à votre org)

**Done quand :**
- [ ] La v0.0.1-preview est listée sur le Marketplace en privé
- [ ] L'install depuis le Marketplace dans votre org de test fonctionne

### T-0.7 — Créer `CLAUDE.md` racine 🟢

📚 contrat avec Claude Code (cf. demande utilisateur)

- [ ] Rédiger `CLAUDE.md` à la racine, listant : règles non-négociables (extraites de la constitution), structure du monorepo, commandes clés, do/don't, liens vers les 4 fichiers spec-kit
- [ ] Format concis (~150-300 lignes max)

**Done quand :**
- [ ] `CLAUDE.md` est mergé sur `main`
- [ ] Une nouvelle session Claude Code ouverte sur le repo le lit automatiquement
...
- [ ] Tests E2E sur instance Cloud et Server

**Done quand :**
- [ ] L'installation depuis zéro fonctionne sur Cloud
- [ ] L'installation depuis zéro fonctionne sur Server 2022
