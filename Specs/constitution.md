...
> Version 0.5.4 — 29 mai 2026
> Principes non-négociables pour le projet TestVault par ATConseil
> Auteur : Alexandre Thibaud — atconseil.info

> **Changelog v0.5.4** : réconciliation no-backend finalisée. §3.2 / §3.2.1 / §3.3 / §3.4 alignées sur §6.0 : le modèle "Cloud-Plus" (Azure Functions ATConseil) est marqué **DEFERRED / non implémenté** (même statut que `plan.md` §7), les appels LLM partent **directement client → provider** (BYOK, aucun proxy serveur). Cette entrée documente aussi rétroactivement la **décision stratégique no-backend du 2026-05-22** (§6.0 « Client-side only », actée après le test E2E Sprint 2.21.1) qui n'avait pas d'entrée changelog. Aucun changement de code. — 2026-05-29.
...
- **MUST** : Node.js **22 LTS** pour les outils de build, le CLI et le SDK (Node.js 20 atteint l'EOL en avril 2026 et le SDK Azure JS impose Node 22 minimum à compter du 9 juillet 2026) — exigence **inconditionnelle**. *(Lorsque le backend Cloud-Plus **DEFERRED** sera implémenté, ses Azure Functions hériteront de la même contrainte Node 22.)*
...
> **⚠️ DEFERRED (décision 2026-05-22, cf. §6.0)** — Argos est désormais une extension **100 % client-side, sans backend hébergé**. Le modèle "Cloud-Plus" (Azure Functions ATConseil) décrit ci-dessous **n'est pas implémenté** et est conservé comme **concept de référence future uniquement** (même statut que `plan.md` §7). Aucune section normative active ne dépend des Azure Functions : les fonctionnalités AI s'exécutent en appel **direct client → provider** (BYOK, cf. §3.2.1 et §6.0).

Toutes les fonctionnalités cœur sont **client-side pures** et identiques Cloud/Server. Les fonctionnalités enrichies (concept Cloud-Plus, **DEFERRED**) auraient été Cloud-exclusives via des Azure Functions hébergées par ATConseil.
...
**Cloud-Plus (Azure Functions ATConseil, Cloud uniquement) — DEFERRED :**
...
- **MUST** : les appels LLM partent **directement du client vers le provider configuré** (BYOK) ; il n'existe **aucun proxy serveur** (cf. §6.0). La clé n'est jamais exposée à un tiers : elle reste dans le contexte d'exécution autorisé de l'extension (ExtensionDataService chiffré) et n'est transmise qu'au provider choisi par le client.
...
| Métadonnées Cloud-Plus (jobs, webhooks) — **DEFERRED (cf. §3.2 / §6.0)** | *Non implémenté ; aurait visé Azure Table Storage ATConseil. Aucun backend actif aujourd'hui.* |
...
- **MUST (DEFERRED, cf. §3.2 / §6.0)** : si le concept Cloud-Plus était un jour implémenté, les données métier transiteraient éphémèrement sans persistance (cache TTL ≤ 1h max, anonymisable). **À ce jour aucun backend n'existe** : aucune donnée métier ne quitte l'instance ADO du client.
...
- **API secondaire (DEFERRED, cf. §3.2 / §6.0 / `plan.md` §7)** : des endpoints REST ATConseil (Azure Functions) étaient prévus pour les features Cloud-Plus. **Non implémenté** ; conservé pour référence future. Les MUST ci-dessous sur les endpoints REST ne s'appliqueront que si ce backend est un jour livré.
...
- [ ] Vérification des API externes (ADO REST, Entra ID, fournisseurs LLM) passées — **N/A tant que le backend Cloud-Plus est DEFERRED** (cf. §3.2 / §6.0). Ne gate la GA que si/quand ce backend est implémenté (nécessiterait alors le déploiement Azure Functions + clés réelles). Les appels LLM étant client-direct (BYOK), aucune clé tierce n'est vérifiable en CI.
