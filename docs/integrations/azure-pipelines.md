...
Set `automationKey` on a Test Case in Argos to enable matching. Results without a matching Test Case are counted as `unmatched` (or fail the task with `strict: true`).
